...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "lovercoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strDNSSeed[][4] = {
    {"nodea", "p2p.lovercoin.net"},
    {"nodeb", "p2p2.lovercoin.net"},
    {"nodec", "p2p3.lovercoin.net"},
    {"noded", "p2p4.lovercoin.net"},
};
...
void ThreadOpenConnections2(void* parg)
{
    ...
    loop
    {
        ...
        if (addrman.size()==0 && (GetTime() - nStart > 60) && !fTestNet)
        {
            ...
            {
                // It'll only connect to one or two seed nodes LVCause once it connects,
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. lovercoin is probably already running."), addrBind.ToString().c_str());
        else
